...
export function ensureWindowsElevatedStartup(
  ...
  try {
    ...
    try {
      ...
      if (!existsSync(path.join(taskDir(), 'clashmetafw-run.exe'))) {
        throw new Error('clashmetafw-run.exe not found')
      }
      execSync('%SystemRoot%\\System32\\schtasks.exe /run /tn clashmetafw-run')
    } catch (error) {
