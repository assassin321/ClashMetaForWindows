...
const helperSocketPath = '/tmp/clashmetafw-helper.sock'
const helperPath = '/Library/PrivilegedHelperTools/clash.meta.helper'
const helperPlistPath = '/Library/LaunchDaemons/clash.meta.helper.plist'
const helperService = 'system/clash.meta.helper'
...
async function isHelperRunning(): Promise<boolean> {
  try {
    ...
    const { stdout } = await execPromise('pgrep -f clash.meta.helper')
...
async function startHelperService(forceRepair = false): Promise<void> {
  const sourceHelper = shellQuote(path.join(resourcesFilesDir(), 'clash.meta.helper'))
  const sourcePlist = shellQuote(path.join(resourcesFilesDir(), 'clash.meta.helper.plist'))
...
async function requestSocketRecreation(): Promise<void> {
  try {
    const execPromise = promisify(exec)
    const shell = `pkill -USR1 -f clash.meta.helper`
