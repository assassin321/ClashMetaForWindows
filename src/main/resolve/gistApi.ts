...
async function createGist(token: string, content: string): Promise<void> {
  ...
  await chromeRequest.post(
    ...
    {
      description: 'Auto Synced Clash Meta For Windows Runtime Config',
      public: false,
      files: { 'clashmetafw.yaml': { content } }
    },
    {
      headers: {
        Accept: 'application/vnd.github+json',
...
export async function getGistUrl(): Promise<string> {
  ...
  const gist = gists.find((gist) => gist.description === 'Auto Synced Clash Meta For Windows Runtime Config')
  if (gist) {
...
export async function exportGistAgeSecretKey(): Promise<boolean> {
  ...
  const { canceled, filePath } = await dialog.showSaveDialog({
    ...
    defaultPath: 'clashmetafw-gist-age-secret-key.txt',
