...
async function installUpdate(version: string): Promise<void> {
  ...
  const fileMap = {
    'win32-x64': `clash-meta-windows-${version}-x64-setup.exe`,
    'win32-ia32': `clash-meta-windows-${version}-ia32-setup.exe`,
    'win32-arm64': `clash-meta-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `clash-meta-macos-${version}-x64.pkg`,
    'darwin-arm64': `clash-meta-macos-${version}-arm64.pkg`
  }
