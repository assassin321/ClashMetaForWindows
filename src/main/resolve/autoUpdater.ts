...
const UPDATE_MANIFEST_URLS: Record<AppUpdateChannel, string> = {
  stable: 'https://github.com/assassin321/ClashMetaForWindows/releases/latest/download/latest.yml',
  rolling: 'https://github.com/assassin321/ClashMetaForWindows/releases/download/rolling/latest.yml'
}
...
export async function downloadAndInstallUpdate(version: string, tag?: string): Promise<void> {
  ...
  const baseUrl = `https://github.com/assassin321/ClashMetaForWindows/releases/download/${releaseTag}/`
  const fileMap: Record<string, string> = {
    'win32-x64': `Clash Meta For Windows-windows-${version}-x64-setup.exe`,
    'win32-arm64': `Clash Meta For Windows-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `Clash Meta For Windows-macos-${version}-x64.pkg`,
    'darwin-arm64': `Clash Meta For Windows-macos-${version}-arm64.pkg`
  }
  ...
  const apiUrl = `https://api.github.com/repos/assassin321/ClashMetaForWindows/releases/tags/${releaseTag}`
