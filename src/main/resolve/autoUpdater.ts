...
async function getGitHubAssetSha256(
  ...
  const res = await chromeRequest.get<GitHubRelease>(
    `https://api.github.com/repos/assassin321/ClashMetaForWindows/releases/tags/${releaseTag}`,
    {
      headers: {
...
export async function checkUpdate(): Promise<IAppVersion | undefined> {
  ...
  const githubUrl =
    'https://github.com/assassin321/ClashMetaForWindows/releases/latest/download/latest.yml'
...
async function installUpdate(version: string): Promise<void> {
  const [{ 'mixed-port': mixedPort = DEFAULT_MIHOMO_PORTS.mixed }, { githubProxy = '' }] =
    await Promise.all([getControledMihomoConfig(), getAppConfig()])
  const githubBase = `https://github.com/assassin321/ClashMetaForWindows/releases/download/v${version}/`
  const fileMap = {
    'win32-x64': `clashmetafw-windows-${version}-x64-setup.exe`,
    'win32-ia32': `clashmetafw-windows-${version}-ia32-setup.exe`,
    'win32-arm64': `clashmetafw-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `clashmetafw-macos-${version}-x64.pkg`,
    'darwin-arm64': `clashmetafw-macos-${version}-arm64.pkg`
  }
