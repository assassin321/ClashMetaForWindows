...
export function mihomoIpcPath(): string {
  if (process.platform === 'win32') {
    return '\\\\.\\pipe\\ClashMetaFW\\mihomo'
  }
  const { core = 'mihomo' } = getAppConfigSync()
  if (core === 'system') {
    return '/tmp/clashmetafw-mihomo-external.sock'
  }
  if (!checkCorePermissionPathSync(mihomoCorePath(core))) {
    return '/tmp/clashmetafw-mihomo-api-noperm.sock'
  }
  return '/tmp/clashmetafw-mihomo-api.sock'
}

export function serviceIpcPath(): string {
  if (process.platform === 'win32') {
    return '\\\\.\\pipe\\clashmetafw\\service'
  }
  return '/tmp/clashmetafw-service.sock'
}
...
export function servicePath(): string {
  const isWin = process.platform === 'win32'
  return path.join(resourcesFilesDir(), `clashmetafw-service${isWin ? '.exe' : ''}`)
}
