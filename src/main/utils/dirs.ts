...
// `clash-meta Safe Storage` Keychain item 或修改正式配置。已打包的 dev 预发行版
// 仍与正式版共享身份和数据，保持原有滚动升级路径。
export function configureAppPaths(): void {
  if (!app.isPackaged) {
    app.setName('clash-meta-dev')
    app.setPath('userData', path.join(app.getPath('appData'), 'clash-meta-dev'))
  }
...
export function logPath(): string {
  ...
  const name = `clash-meta-${year}-${month}-${day}`
