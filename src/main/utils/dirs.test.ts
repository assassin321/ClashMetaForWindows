...
beforeEach(() => {
  ...
  Object.assign(paths, {
    appData: APP_DATA,
    userData: path.join(APP_DATA, 'clash-meta'),
    home: HOME,
    exe: EXE
  })
  setPath.mockClear()
...
describe('configureAppPaths', () => {
  it('isolates an unpackaged local development app', async () => {
    ...
    expect(setName).toHaveBeenCalledWith('clash-meta-dev')
    expect(paths.userData).toBe(path.join(APP_DATA, 'clash-meta-dev'))
  })
  ...
  it('leaves packaged stable and dev-release builds on production paths', async () => {
    ...
    expect(paths.userData).toBe(path.join(APP_DATA, 'clash-meta'))
  })
