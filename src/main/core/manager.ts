...
const coreProcessNames = ['mihomo', 'mihomo-alpha'] as const
...
function coreHookDir(): string {
  if (process.platform === 'win32' && process.env.ProgramData) {
    return path.join(process.env.ProgramData, 'clashmetafw', 'core-hooks')
  }
...
export const getMihomoIpcPath = (): string => {
  if (process.platform === 'win32') {
    ...
    return isAdmin
      ? `\\\\.\\pipe\\ClashMetaForWindows\\mihomo-admin-${sessionId}-${processId}`
      : `\\\\.\\pipe\\ClashMetaForWindows\\mihomo-user-${sessionId}-${processId}`
  }

  const uid = process.getuid?.() || 'unknown'
  const processId = process.pid
  return `/tmp/clashmetafw-${uid}-${processId}.sock`
}
