...
export async function cleanupUnixSockets(): Promise<void> {
  ...
  const legacyPaths = [
    '/tmp/clashmetafw.sock',
    '/tmp/clashmetafw-admin.sock',
    `/tmp/clashmetafw-${uid}.sock`
  ]
  ...
  // Current getMihomoIpcPath() produces /tmp/clashmetafw-<uid>-<pid>.sock,
  ...
  // (another Clash Meta For Windows window from a different user, an admin-elevated session,
  // etc.) that legitimately owns its own socket.
  const currentPrefix = `clashmetafw-${uid}-`
  try {
