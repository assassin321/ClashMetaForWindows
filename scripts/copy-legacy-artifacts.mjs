 ...
 * 将 clash-meta 重命名为 clash-meta，用于更新检测兼容性
 */
...
for (const file of files) {
  if (file.includes('clash-meta') && !file.endsWith('.sha256')) {
    const newFileName = file.replace('clash-meta', 'clash-meta')
...
if (copiedCount > 0) {
  ...
  finalFiles.forEach((file) => {
    if (file.includes('clash-meta') || file.includes('clash-meta')) {
      const isLegacy = file.includes('clash-meta')
      console.log(`   ${isLegacy ? '🔄' : '📦'} ${file}`)
    }
  })

  console.log('   📦 = 原始文件 (clash-meta)')
  console.log('   🔄 = 兼容文件 (clash-meta)')
} else {
  console.log('ℹ️  没有找到需要复制的 clash-meta 文件')
}
