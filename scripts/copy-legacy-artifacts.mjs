 ...
 * 将 clashmetafw 重命名为 clashmetafw，用于更新检测兼容性
 */
...
for (const file of files) {
  if (file.includes('clashmetafw') && !file.endsWith('.sha256')) {
    const newFileName = file.replace('clashmetafw', 'clashmetafw')
...
if (copiedCount > 0) {
  ...
  finalFiles.forEach((file) => {
    if (file.includes('clashmetafw') || file.includes('clashmetafw')) {
      const isLegacy = file.includes('clashmetafw')
      console.log(`   ${isLegacy ? '🔄' : '📦'} ${file}`)
    }
  })

  console.log('   📦 = 原始文件 (clashmetafw)')
  console.log('   🔄 = 兼容文件 (clashmetafw)')
} else {
  console.log('ℹ️  没有找到需要复制的 clashmetafw 文件')
}
